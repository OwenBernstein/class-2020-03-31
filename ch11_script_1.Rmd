...
# This value means that there is a positive correlation between education and poverty. As the percentage of people with less education increases the percentage of people in poverty increases. 
...
ggplot(county, aes(less_than_hs, poverty)) +geom_point() +geom_smooth(method = lm)
...
ggplot(county, aes(less_than_hs, poverty, color = region)) +
  geom_point() +
...
avg_pov <- edu %>% 
  ...
  summarize(mean_pov = mean(poverty))
avg_pov
...
Treatment = good_education or not
Units = county
outcomes = poverty

does good education cause a change in poverty
...
# The expected poverty level at 0% less_than_hs (6.63). For every 1% increase in less_than_hs expect a 0.64 increase in % poverty.  
```
...
boot <- rep_sample_n(size = 3129, reps = 100, tbl = county) %>% 
  group_by(replicate) %>% 
  nest()
```
